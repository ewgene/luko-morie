<template>
  ...
  //    const botOffset = ref(0)
      ...
      const ancorLr05 = computed(()=> {
          return lr_05.value?.$el.offsetLeft }
      )

      function insertComponent(parent:HTMLElement|any, src:string|Component, ancor:number, wdth:number, direction:string) {

        console.log(ancorLr05.value)

        if(Math.round(lr_05.value?.$el.getBoundingClientRect().x) === 0 
          || Math.floor(lr_05.value?.$el.getBoundingClientRect().right) === window.innerWidth) {
          let container:HTMLElement|any = parent
          console.log(container, src, ancor, wdth, direction)
          
          if(direction == 'right') {
            let html = "<li id='copy-left' class='layer' style='width:"+wdth+"px'><svg viewBox='0 0 7871 650' xmlns='http://www.w3.org/2000/svg'><rect width='7871' height='650' fill='transparent' /></li></nobr>"
